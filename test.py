...
class Player(object):
    ...
    def draw(self, window):
        if self.visible:
            ...
            if apple.y - apple.hight <= nefton.hitbox[1] and apple.y + apple.hight >= nefton.hitbox[1]:
                if apple.x - apple.width <= nefton.hitbox[0] and apple.x + apple.width >= nefton.hitbox[0]:
                    nefton.hit()
                    nefton.pop(nefton.index(nefton))
...
class Apple(object):
    ...
    def draw(self, window):
        ...
        if apple.count == apple.next_speed:
...
def redraw_game_window():
    ...
    text = font_apple.render('# apples: ' + str(apple.count), 1, (255, 255, 255))
